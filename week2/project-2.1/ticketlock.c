...
_Atomic(int) next_ticket;
_Atomic(int) now_serving;
 ...
 */
void lock(int tid) {
    // store current ticket number and increment next ticket
    int ticket_num = atomic_fetch_add(&next_ticket, 1);

    // Print out the number of the ticket that was just grabbed:
    printf("Thread %d: Grabbed ticket %d\n", tid, ticket_num);

    // spin while ticket isn't being served
    while(now_serving != ticket_num);
}
