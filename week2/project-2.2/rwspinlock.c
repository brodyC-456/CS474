...
void read_lock(void)
{
    int expected = reader_count;
    do {
        expected = reader_count;
        while(((lock >> WRITER_BIT_POS) & 1) == 1);
...
void write_lock()
{
    ...
    do{
        expected = 0; 
    } while(!atomic_compare_exchange_weak(&lock, &expected, 1<<WRITER_BIT_POS));
}
