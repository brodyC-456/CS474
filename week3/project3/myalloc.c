...
static int first_myalloc_call = 1;
...
void *make_block(struct block *b, int actual_size, int padded_struct_block_size){
    // Make a new block
    struct block *new = PTR_OFFSET(b, actual_size + padded_struct_block_size);

    new->size = b->size - actual_size - padded_struct_block_size;
    new->in_use = 0;
    new->next = b->next;
...
void initialize_head(int padded_struct_block_size){
    head = mmap(NULL, MEM_SIZE, PROT_READ|PROT_WRITE, MAP_ANON|MAP_PRIVATE, -1, 0);
    int data_size = MEM_SIZE - padded_struct_block_size;
    
    // Initializing head struct values
    head->size = data_size;
    head->in_use = 0;
    head->next = NULL;
}
...
{
    // Padded size variables
    int actual_size = PADDED_SIZE(size);
    int padded_struct_block_size = PADDED_SIZEOF(struct block);
    
    // On the first call, create the memory region
    if (first_myalloc_call == 1){
        initialize_head(padded_struct_block_size);
        first_myalloc_call = 0;
    }

    // Making a temporary variable so the head isn't directly altered
    struct block *temp_head = head;

    while(temp_head != NULL){
        if (temp_head->size >= actual_size && temp_head->in_use == 0){
            if(temp_head->size < actual_size + padded_struct_block_size){
                temp_head -> in_use = 1;
                return PTR_OFFSET(temp_head, padded_struct_block_size);
            }
            else{
                return make_block(temp_head, actual_size, padded_struct_block_size);
            }
...
void myfree(void *p)
{
    // Compute the location of the block then set it to not in use
    int padded_block_size = PADDED_SIZEOF(struct block);
    struct block *b = (struct block *)((char *)p - padded_block_size);
    b->in_use = 0;

    struct block *temp_head = head;
    while(temp_head != NULL && temp_head->next != NULL){
        struct block *next = temp_head->next;
        if(temp_head->in_use == 0 && next->in_use == 0){
            temp_head->size += next->size + padded_block_size;
